...
servo1 = Servo(chan1,set = S1,gain_p = 1.0,gain_i = 0.0)
servo2 = Servo(chan2,set = S2,gain_p = 1.0,gain_i = 0.0)
servo3 = Servo(chan3,set = S3,gain_p = 2.0,gain_i = 0.05)
    ...
    while(True):
        ...
        if(publish and counter%100==0):
            try:
                ...
                if(servo1.enable):
                    temp1 = get_temp(chan1) - S1
                if(servo2.enable):
                    temp2 = get_temp(chan2) - S2
                if(servo3.enable):
                    temp3 = get_temp(chan3) - S3
                if(servo4.enable):
                    temp4 = get_temp(chan4) - S4
